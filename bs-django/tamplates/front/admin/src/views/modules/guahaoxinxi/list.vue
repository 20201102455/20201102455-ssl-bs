			...
			<el-form class="center-form-pv" :style='{"border":"0px solid #ceddee","minHeight":"100px","padding":"10px 0px","boxShadow":"0 0px 0px 0px rgba(100,100,100,.05)","margin":"0px","overflow":"hidden","borderRadius":"20px 20px","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"center"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"center","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">挂号单号</label>
						<el-input v-model="searchForm.guahaodanhao" placeholder="挂号单号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="就诊状态" prop="jiuzhenzhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">就诊状态</label>
						<el-select clearable v-model="searchForm.jiuzhenzhuangtai" placeholder="请选择就诊状态" >
							<el-option v-for="(item,index) in jiuzhenzhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"auto","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('guahaoxinxi','新增')" type="success" @click="addOrUpdateHandler()">
					...
					<el-button class="del" v-if="isAuth('guahaoxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('guahaoxinxi','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
			...
			<div :style='{"padding":"0px","boxShadow":"inset 0px 0px 0px 0px #E8EFF6","borderColor":"#d2d0d0","margin":"0 0 0px","borderRadius":"0px","background":"rgba(255,255,255,.8)","borderWidth":"1px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="挂号数量">
						<template slot-scope="scope">
							{{scope.row.guahaoshuliang}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiuzhenzhuangtai"
						label="就诊状态">
						<template slot-scope="scope">
							{{scope.row.jiuzhenzhuangtai}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="yonghuming"
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('guahaoxinxi','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="shhf" label="回复内容"></el-table-column>
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('guahaoxinxi','审核')" prop="sfsh" label="回复">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('guahaoxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="btn8" v-if="isAuth('guahaoxinxi','添加就诊')" @click="binglixinxiCrossAddOrUpdateHandler(scope.row,'cross','','jiuzhenzhuangtai','该患者已就诊！','已就诊,未就诊'.split(',')[0])" type="success">
							...
							<el-button class="edit" v-if=" isAuth('guahaoxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('guahaoxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import binglixinxiCrossAddOrUpdate from "../binglixinxi/add-or-update";
	export default {
		...
		methods: {
			...
			binglixinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.binglixinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','guahaoxinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.binglixinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
			...
			payHandler(row){
				this.$storage.set('paytable','guahaoxinxi');
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.guahaodanhao!='' && this.searchForm.guahaodanhao!=undefined){
            params['guahaodanhao'] = '%' + this.searchForm.guahaodanhao + '%'
          }
           ...
           if(this.searchForm.jiuzhenzhuangtai!='' && this.searchForm.jiuzhenzhuangtai!=undefined){
            params['jiuzhenzhuangtai'] = this.searchForm.jiuzhenzhuangtai
          }
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          guahaodanhao: row.guahaodanhao,
          yishenggonghao: row.yishenggonghao,
          yishengxingming: row.yishengxingming,
          keshimingcheng: row.keshimingcheng,
          guahaojine: row.guahaojine,
          guahaoshuliang: row.guahaoshuliang,
          jiuzhenzhuangtai: row.jiuzhenzhuangtai,
