<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"20px 0 30px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #ceddee","minHeight":"100px","padding":"10px 0px","boxShadow":"0 0px 0px 0px rgba(100,100,100,.05)","margin":"0px","overflow":"hidden","borderRadius":"20px 20px","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"center"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"auto","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('zuozhenxinxi','新增')" type="success" @click="addOrUpdateHandler()">
					...
					<el-button class="del" v-if="isAuth('zuozhenxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"0px","boxShadow":"inset 0px 0px 0px 0px #E8EFF6","borderColor":"#d2d0d0","margin":"0 0 0px","borderRadius":"0px","background":"rgba(255,255,255,.8)","borderWidth":"1px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="yishengzhaopian" width="200" label="医生照片">
						<template slot-scope="scope">
							<div v-if="scope.row.yishengzhaopian">
								<img v-if="scope.row.yishengzhaopian.substring(0,4)=='http'" :src="scope.row.yishengzhaopian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.yishengzhaopian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="shanzhangfangmian"
						label="擅长方面">
						<template slot-scope="scope">
							{{scope.row.shanzhangfangmian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="guahaojine"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zuozhenxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="btn8" v-if="isAuth('zuozhenxinxi','在线咨询')" @click="zixunxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
							...
							<el-button class="btn8" v-if="isAuth('zuozhenxinxi','在线挂号')" @click="guahaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"24px","position":"absolute","color":"#327e33","left":"0","height":"auto"}'></span>
								在线挂号
							</el-button>
							<el-button class="edit" v-if=" isAuth('zuozhenxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('zuozhenxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<guahaoxinxi-cross-add-or-update v-if="guahaoxinxiCrossAddOrUpdateFlag" :parent="this" ref="guahaoxinxiCrossaddOrUpdate"></guahaoxinxi-cross-add-or-update>
...
import guahaoxinxiCrossAddOrUpdate from "../guahaoxinxi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				guahaoxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			zixunxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','zuozhenxinxi');
			...
			guahaoxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.guahaoxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','zuozhenxinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.guahaoxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.guahaoxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	zuozhenxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'zuozhenxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
