<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"20px 0 30px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #ceddee","minHeight":"100px","padding":"10px 0px","boxShadow":"0 0px 0px 0px rgba(100,100,100,.05)","margin":"0px","overflow":"hidden","borderRadius":"20px 20px","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"center"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"center","order":"2"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">医生职称</label>
						<el-input v-model="searchForm.yishengzhicheng" placeholder="医生职称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yisheng','添加坐诊')" @click="zuozhenxinxiCrossAddOrUpdateHandler(scope.row,'cross','','[1]','您已添加过该医生坐诊信息！')" type="success">
		...
		<zuozhenxinxi-cross-add-or-update v-if="zuozhenxinxiCrossAddOrUpdateFlag" :parent="this" ref="zuozhenxinxiCrossaddOrUpdate"></zuozhenxinxi-cross-add-or-update>
...
import zuozhenxinxiCrossAddOrUpdate from "../zuozhenxinxi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				zuozhenxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			zuozhenxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.zuozhenxinxiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.zuozhenxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.zuozhenxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.yishengzhicheng!='' && this.searchForm.yishengzhicheng!=undefined){
            params['yishengzhicheng'] = '%' + this.searchForm.yishengzhicheng + '%'
          }
      this.$http({
        url: "yisheng/page",
        method: "get",
